...
func TestNewReferenceList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewReferenceList(tt.input)
			...
			if len(got) != len(tt.want) {
				t.Errorf("NewReferenceList() returned %d references, want %d", len(got), len(tt.want))
				return
			}
			for i, ref := range got {
				if !reflect.DeepEqual(ref, tt.want[i]) {
					t.Errorf("NewReferenceList() reference[%d] = %v, want %v", i, ref, tt.want[i])
				}
			}
		})
	}
