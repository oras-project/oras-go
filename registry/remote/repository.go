	...
	update := func(referrerChanges []referrerChange) error {
		...
		if !skipDelete {
			if err := s.repo.delete(ctx, oldIndexDesc, true); err != nil {
				return DanglingReferrerIndexErr{fmt.Errorf("failed to delete dangling referrers index %s for referrers tag %s: %w", oldIndexDesc.Digest.String(), referrersTag, err)}
			}
		}
		return nil
	}
