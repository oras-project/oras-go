...
// It supports JWT tokens used in bearer auth and returns a zero time for
// tokens that don't contain expiration information (like basic auth tokens).
// For JWT tokens without an expiration claim, it returns a default expiration
// of 60 seconds from now.
...
// It includes a 10-second grace period to avoid using tokens that are
// about to expire.
func (te *tokenEntry) isExpired() bool {
	...
	return time.Now().Add(10 * time.Second).After(te.expiresAt)
}
