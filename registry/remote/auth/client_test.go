	...
	client := &Client{
		Credential: func(ctx context.Context, reg string) (Credential, error) {
			if reg != originURL.Host {
				return EmptyCredential, fmt.Errorf("registry mismatch: got %v, want %v", reg, originURL.Host)
			}
...
func TestClient_Do_Basic_Auth_SameOriginRedirect(t *testing.T) {
	...
	client := &Client{
		Credential: func(ctx context.Context, reg string) (Credential, error) {
			if reg != tsURL.Host {
				return EmptyCredential, fmt.Errorf("registry mismatch: got %v, want %v", reg, tsURL.Host)
			}
			return Credential{Username: username, Password: password}, nil
		},
	}
