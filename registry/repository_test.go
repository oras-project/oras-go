...
// mockReadableRepository is a mock implementation of ReadableRepository for testing.
type mockReadableRepository struct{}

func (m *mockReadableRepository) Fetch(ctx context.Context, target ocispec.Descriptor) (io.ReadCloser, error) {
	return io.NopCloser(bytes.NewReader([]byte("content"))), nil
}

func (m *mockReadableRepository) Resolve(ctx context.Context, reference string) (ocispec.Descriptor, error) {
	return ocispec.Descriptor{Digest: digest.FromString("test")}, nil
}

func (m *mockReadableRepository) FetchReference(ctx context.Context, reference string) (ocispec.Descriptor, io.ReadCloser, error) {
	desc := ocispec.Descriptor{Digest: digest.FromString("test")}
	return desc, io.NopCloser(bytes.NewReader([]byte("content"))), nil
}

// mockWritableRepository is a mock implementation of WritableRepository for testing.
type mockWritableRepository struct{}

func (m *mockWritableRepository) Push(ctx context.Context, expected ocispec.Descriptor, content io.Reader) error {
	return nil
}

func (m *mockWritableRepository) Tag(ctx context.Context, desc ocispec.Descriptor, reference string) error {
	return nil
}

func (m *mockWritableRepository) PushReference(ctx context.Context, expected ocispec.Descriptor, content io.Reader, reference string) error {
	return nil
}

// mockReadOnlyRepository is a mock implementation of ReadOnlyRepository for testing.
type mockReadOnlyRepository struct {
	mockReadableRepository
}

func (m *mockReadOnlyRepository) Exists(ctx context.Context, target ocispec.Descriptor) (bool, error) {
	return true, nil
}

func (m *mockReadOnlyRepository) Referrers(ctx context.Context, desc ocispec.Descriptor, artifactType string, fn func(referrers []ocispec.Descriptor) error) error {
	return fn(nil)
}

func (m *mockReadOnlyRepository) Tags(ctx context.Context, last string, fn func(tags []string) error) error {
	return fn(nil)
}

// TestReadableRepository verifies that the ReadableRepository interface
// can be implemented and used correctly.
func TestReadableRepository(t *testing.T) {
	var repo ReadableRepository = &mockReadableRepository{}

	ctx := context.Background()

	// Test Fetch
	_, err := repo.Fetch(ctx, ocispec.Descriptor{})
	if err != nil {
	...
	// Test FetchReference
	_, _, err = repo.FetchReference(ctx, "latest")
	if err != nil {
		t.Errorf("FetchReference() error = %v", err)
	}
}

// TestWritableRepository verifies that the WritableRepository interface
// can be implemented and used correctly.
func TestWritableRepository(t *testing.T) {
	var repo WritableRepository = &mockWritableRepository{}

	ctx := context.Background()

	// Test Push
	err := repo.Push(ctx, ocispec.Descriptor{}, bytes.NewReader(nil))
	if err != nil {
		t.Errorf("Push() error = %v", err)
	}

	// Test Tag
	err = repo.Tag(ctx, ocispec.Descriptor{}, "latest")
	if err != nil {
		t.Errorf("Tag() error = %v", err)
	}

	// Test PushReference
	err = repo.PushReference(ctx, ocispec.Descriptor{}, bytes.NewReader(nil), "latest")
	if err != nil {
		t.Errorf("PushReference() error = %v", err)
	}
}

// TestReadOnlyRepository verifies that the ReadOnlyRepository interface
// can be implemented and used correctly.
func TestReadOnlyRepository(t *testing.T) {
	var repo ReadOnlyRepository = &mockReadOnlyRepository{}

	ctx := context.Background()

	// Test inherited ReadableRepository methods
	_, err := repo.Fetch(ctx, ocispec.Descriptor{})
	if err != nil {
		t.Errorf("Fetch() error = %v", err)
	}
	...
	// Test Exists
	exists, err := repo.Exists(ctx, ocispec.Descriptor{})
	if err != nil {
	...
	// Test Referrers
	err = repo.Referrers(ctx, ocispec.Descriptor{}, "", func(referrers []ocispec.Descriptor) error {
		return nil
	})
	if err != nil {
...
// TestInterfaceComposition verifies that the interfaces compose correctly.
func TestInterfaceComposition(t *testing.T) {
	// Verify ReadableRepository can be assigned from ReadOnlyRepository
	var readOnly ReadOnlyRepository = &mockReadOnlyRepository{}
	var readable ReadableRepository = readOnly
	_ = readable

	// Verify the mock types satisfy their interfaces (compile-time check)
	var _ ReadableRepository = (*mockReadableRepository)(nil)
	var _ WritableRepository = (*mockWritableRepository)(nil)
	var _ ReadOnlyRepository = (*mockReadOnlyRepository)(nil)
}
