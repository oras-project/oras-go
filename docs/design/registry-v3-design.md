...
- `signature.NewSignedByVerifierFromConfig(cfg.RegistriesDConfig, scope)` — creates a `*signature.DefaultSignedByVerifier` via longest-prefix matching on the registries.d YAML keys
...
| `ForceAttemptOAuth2` flag | `bool` field on `auth.Client` | removed; OAuth2 is the default, opt into the legacy distribution flow with `TokenFetcher` |
