...
func TestExtendedCopy_FullCopy(t *testing.T) {
	...
	// test extended copy
	gotDesc, err := oras.ExtendedCopy(ctx, src, ref, dst, "")
	if err != nil {
...
func TestExtendedCopyGraph_FullCopy(t *testing.T) {
	...
	dst := memory.New()
	if err := oras.ExtendedCopyGraph(ctx, src, dst, descs[0]); err != nil {
	...
	dst = memory.New()
	if err := oras.ExtendedCopyGraph(ctx, src, dst, descs[4]); err != nil {
		t.Fatalf("CopyGraph() error = %v, wantErr %v", err, false)
	}
	...
	dst = memory.New()
	if err := oras.ExtendedCopyGraph(ctx, src, dst, descs[14]); err != nil {
		t.Fatalf("CopyGraph() error = %v, wantErr %v", err, false)
	}
